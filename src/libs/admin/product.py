...
def product_handler(event, context):
  try:
    ...
    if existing_product := products.find_one({"name": body.get("name")}):
      return {
        "statusCode": 400,
    ...
    for variant in variants_body:
      ...
      variant_id = variants.insert_one(variant).inserted_id
...
def update_product_handler(event, context):
  try:
    ...
    if updated_product := db.subcategories.find_one_and_update(
